...
export default function Sidebar({ activePage, setActivePage, userEmail, onLogout, isOpen, onClose, collapsed, onToggleCollapse }) {
  const navItems = [
    ...
    { id: 'coding', label: 'Code Lab', icon: 'code' },
  ...
  const sidebarClasses = `
    fixed h-screen w-64 left-0 top-0 flex flex-col py-6 glass-panel border-r border-outline-variant/30 z-50 justify-between transition-transform duration-300 ease-out
    ${isOpen ? 'translate-x-0' : '-translate-x-full'}
    ${!collapsed ? 'md:translate-x-0' : 'md:-translate-x-full'}
  `;

  return (
    <>
      {/* Desktop hamburger — fixed at top-left, always accessible */}
      <button
        onClick={onToggleCollapse}
        className="fixed top-4 left-4 z-[60] hidden md:flex items-center justify-center w-9 h-9 rounded-lg hover:bg-surface-container-high/40 text-on-surface-variant hover:text-on-surface transition-colors"
      ...
      <nav className={sidebarClasses}>
        <div>
          {/* Brand */}
          <div className="pl-[3.5rem] pr-6 mb-10">
