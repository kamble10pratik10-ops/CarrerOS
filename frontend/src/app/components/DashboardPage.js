...
export default function DashboardPage({ onNavigate }) {
  ...
  if (loading) {
    return (
      <div className="page-transition p-4 md:p-8 space-y-6">
  ...
  return (
      ...
      {/* Code Lab Progress */}
      <div>
        <h2 className="text-xs font-extrabold text-muted uppercase tracking-widest mb-4.5">Code Lab</h2>
        <button onClick={() => onNavigate('coding')} className="card w-full text-left p-6 cursor-pointer group">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              <div className="w-12 h-12 rounded-xl bg-gradient-to-tr from-secondary/20 to-primary/20 border border-secondary/20 flex items-center justify-center shadow-lg shadow-secondary/5">
                <span className="material-symbols-outlined text-secondary text-2xl">code</span>
              </div>
              <div>
                <p className="text-sm font-bold text-on-surface group-hover:text-primary transition-colors">AI Coding Mentor</p>
                <p className="text-xs text-muted mt-0.5">Practice DSA problems with an AI pair programmer</p>
              </div>
            </div>
