...
export default function AuthPage({ onAuthSuccess, onBack, onForgotPassword }) {
  const [mode, setMode] = useState('login');
  const [form, setForm] = useState({ email: '', mobile: '', password: '', recoveryQuestion: 'What was the name of your first school?', customQuestion: '', recoveryAnswer: '' });
  ...
  const handleSubmit = async (e) => {
    ...
    setError('');
    if (mode === 'signup') {
      const q = form.recoveryQuestion === 'Custom question' ? form.customQuestion : form.recoveryQuestion;
      if (!q?.trim() || !form.recoveryAnswer?.trim()) {
        setError('Recovery question and answer are required');
        setLoading(false);
        return;
      }
    }
    ...
    const body = mode === 'login'
      ...
      : { 
          ...
          recoveryQuestion: form.recoveryQuestion === 'Custom question' ? form.customQuestion : form.recoveryQuestion,
            ...
            <form onSubmit={handleSubmit} className="auth-form">
              ...
              {mode === 'signup' && (
                <div className="auth-field auth-field-animate">
                  ...
                  <div className="auth-input-wrap" style={{ gap: 8 }}>
                    ...
                    <select className="auth-input auth-input-select" value={countryCode} onChange={(e) => { setCountryCode(e.target.value); setError(''); }} style={{ minWidth: '85px', maxWidth: '120px', flexShrink: 0 }}>
                      <option key="selected" value={countryCode}>{countryCode}</option>
                      {countryCodes.map(cc => (
                        <option key={cc.code} value={cc.code}>{cc.code} {cc.country}</option>
                      ))}
                    </select>
                    <input className="auth-input auth-input-tel" type="tel" name="mobile" placeholder="9999999999" value={form.mobile} onChange={handleChange} required autoComplete="tel" style={{ flex: 1, minWidth: 0 }} />
                  </div>
                </div>
              )}
              ...
              {mode === 'login' && (
                <div style={{ textAlign: 'right', marginTop: '-8px', marginBottom: '8px' }}>
                  <button type="button" onClick={onForgotPassword} style={{ background: 'none', border: 'none', color: '#8b5cf6', fontSize: '13px', cursor: 'pointer', fontWeight: 500 }} className="hover:text-purple-400 transition-colors">
                    Forgot Password?
                  </button>
                </div>
              )}
              ...
              {mode === 'signup' && (
                <>
                  <div className="auth-field auth-field-animate">
                    <label className="auth-label">Recovery Question</label>
                    <div className="auth-input-wrap">
                      <select className="auth-input auth-input-select" name="recoveryQuestion" value={form.recoveryQuestion} onChange={handleChange} required style={{ width: '100%', appearance: 'none' }}>
                  ...
                  {form.recoveryQuestion === 'Custom question' && (
                    <div className="auth-field auth-field-animate">
                      <label className="auth-label">Your Custom Question</label>
                      <div className="auth-input-wrap">
                        <input className="auth-input" type="text" name="customQuestion" placeholder="Enter your custom question" value={form.customQuestion} onChange={handleChange} required />
                      </div>
                    </div>
                  )}

                  <div className="auth-field auth-field-animate">
                    <label className="auth-label">Recovery Answer</label>
                    <div className="auth-input-wrap">
                      <input className="auth-input" type="text" name="recoveryAnswer" placeholder="Your secret answer" value={form.recoveryAnswer} onChange={handleChange} required />
                    </div>
                  </div>
                </>
              )}
